...
var loopPasos = (new Tone.Loop((random) => {
  sampler.triggerAttackRelease("C-1", (random + 0.5) * 8, (random + 0.5) * 4);
}, 4).start(random * 2).loop = true);

var loopPads = (new Tone.Loop((random) => {
  sampler2.triggerAttackRelease("C1", (random + 0.5) * 4, (random + 0.5) * 2);
}, 1).start(random).loop = true);

var loopClicks = (new Tone.Loop((random) => {
  clicks.triggerAttackRelease("C0", (random + 0.5) * 8, (random + 0.5) * 8);
